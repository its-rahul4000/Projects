...
from config.settings import (
    ...
    ENV_SMTP_PASS, ENV_SMTP_FROM_NAME, APP_NAME,
)
...
def test_smtp_connection() -> tuple[bool, str]:
    """
    Attempt a real SMTP connection (login only, no email sent).
    Returns (success, message) suitable for display in the UI.
    """
    ...
    try:
        ...
        return False, f"Authentication failed for {cfg['user']}. Check your SMTP_USER and SMTP_PASS."
...
def send_pdf_report_email(
    ...
    msg["From"] = f"{cfg['from_name']} <{cfg['user']}>"
    msg["To"] = to_email
    msg["Subject"] = f"[SECURITY REPORT] Log Analysis Report - {datetime.date.today()}"
...
Analysis Date:   {datetime.datetime.utcnow().strftime('%Y-%m-%d %H:%M:%S')} UTC
